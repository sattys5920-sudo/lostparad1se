 ...
 * 이름 하나를 적으라는 말.
 *
 * **지금 화면에는 안 뜬다.** 종이에는 `투 표 용 지` 와 이름밖에 없고,
 * 아래쪽 절반(`VOTE_FOOT`)만 인쇄돼 있다. 앞 절반은 꺼져 있는 단추가
 * `이름을 고르세요` 로 대신 말한다. 문장은 남겨 둔다 — 종이 규격이
 * 다시 바뀌면 이 자리로 돌아온다.
 */
 ...
 * 종이 맨 아래에 인쇄된 한 줄.
 *
 * **이유를 묻지 않는다는 것이 이 투표의 전부다.** 적는 칸이 하나뿐인
 * 종이에서 이 문장은 배려처럼 읽히지만, 실제로는 아무도 책임지지
 * 않아도 된다는 뜻이다.
 */
export const VOTE_FOOT = '왜인지는 적지 않아도 됩니다'

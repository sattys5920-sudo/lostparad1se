...
// 그 값은 아무에게도 안 간다 — 심은 운영자에게도. 언제 열매가 될지를
// 알면 화분 앞에 서서 기다리는 일이 없어지고, 그러면 정원에 오갈
// 이유도 없어진다.
//
// **심는 것은 운영자다.** 씨앗 상자도 씨앗도 없다 — 게시판의 심부름과
// 같다. 무엇이 언제 자라기 시작할지가 운영자의 손에 있어야, 정원이
// 판의 흐름과 같이 움직인다.
...
/**
 * 화분 여덟 자리. **맵 데이터에 박혀 있다.**
 *
 * 정원은 1층 x21..30, y98..105 다 — **전개도 좌표**이고, board.ts 가
 * 층을 위아래로 쌓아 놓은 자리다. 두 줄로 넷씩 놓는다.
 *
 * 씨앗 상자는 없앴다. 심는 것은 운영자가 하고, 사람이 정원에 가는
 * 이유는 **자란 것을 보러**다.
 *
 * 자리가 맞는지는 눈으로 안 본다 — 시험이 지도에 물어본다.
 */

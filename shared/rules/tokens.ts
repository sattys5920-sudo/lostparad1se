// 행동 토큰 — 시간이 흘러야 찬다.
//
// **이 규칙은 판에서 걷어냈다. 지금은 봇 시뮬레이터만 쓴다.**
//
// 자유 시간에 값을 치르는 일이 없어지면서(생산·공부가 페이즈로 옮겨
// 갔다) 이 주머니는 아무도 안 쓰는 채로 시간마다 차고만 있었다. 이제
// 토큰은 **페이즈가 열릴 때만** 들어온다 — TeamDoc.phaseTokens 하나뿐
// 이고, 채우는 자리는 openPhase 의 nextWallet 한 군데다.
//
// shared/sim 이 아직 이 모형으로 돈다. 시뮬레이터를 페이즈 모형으로
// 다시 짜거나 걷어낼 때 이 파일도 같이 간다 — 그때까지 **판의 규칙으로
// 읽으면 안 된다.**
//

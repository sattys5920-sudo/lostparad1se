 ...
 * 뽑히면 모두에게 알린다.
 *
 * **팀장은 비밀이 아니다.** 숨기면 규칙이 성립하지 않는다 — 팀장은
 * 투명인간 투표에서 적을 수 없는데, 다른 팀 팀장이 누구인지 모르면
 * 적어 본 뒤에야 물린다. 그 물림 하나하나가 「저 사람이 팀장이다」를
 * 알려 주므로, 숨긴 것이 숨겨지지도 않으면서 화면만 막히는 셈이었다.
 */
export const announceCaptain = (team: TeamId, name: string): string =>
  `오늘 ${team}팀 팀장은 ${name}입니다.`

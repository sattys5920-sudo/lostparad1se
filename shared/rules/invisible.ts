 ...
 *
 * 가르는 선은 하나다 — **남이 나를 봐야 되는 일만 막힌다.** 거래도
 * 이적도 쪽지 건네기도 호출도 마주 보고 하는 일이라 안 된다.
 *
 * 혼자 하는 일은 전부 된다. 걷고, 서고, 벌고, 산다. 지워졌다고
 * 하루를 통째로 빼앗지는 않는다 — 없는 사람으로 지내는 것이지
 * 아무것도 못 하는 사람이 되는 것이 아니다.
 */

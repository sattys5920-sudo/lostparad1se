...
describe('묻힌 칸', () => {
  it('열두 곳이다', () => {
    expect(MEMORY_TILES).toHaveLength(12)
  })

  it('관문 4 · 교차로 3 · 핵심 4 · 2-3 교실 1이다', () => {
    const byTier = MEMORY_TILES.reduce<Record<string, number>>((acc, id) => {
      const t = TILE_BY_ID[id].tier
      acc[t] = (acc[t] ?? 0) + 1
      return acc
    }, {})
    expect(byTier).toEqual({ gate: 4, cross: 3, core: 4, plaza: 1 })
  })
...
describe('읽을 수 있는 것', () => {
  ...
  it('끝나면 열두 장면 모두 열린다', () => {
    expect(memoriesFor(opened, 'C', true)).toHaveLength(12)
  })

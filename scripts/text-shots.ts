...
import { MAKERS, TECH_TILE, LAB_TILE } from '../shared/rules/trap'
...
const SITES: [string, string][] = [['전', 'http://127.0.0.1:8898/lostparad1se'], ['후', 'http://127.0.0.1:8899/lostparad1se']]
let SITE = SITES[1][1]
const OUT = '/tmp/claude-0/textshots'
...
/** 한 판을 차리고 네 화면을 찍는다. 같은 대본을 두 빌드에 돌린다 */
async function run(tag: string, site: string, browser: import('playwright').Browser): Promise<void> {
  SITE = site
  const game = `xs${Date.now()}`
  ...
  // ── 게시판. 2-3 교실에서 2층 서쪽 복도까지 걸어간다 ──
  const board = BOARDS.find((b) => b.id === 'f2w')!
  await must('hostPostErrand', host, { gameId: game, specId: 'sheet', boardId: board.id, to: 'broadcastRoom' })
  await walkTo(page, game, meUid, board.cell, '게시판')
  await page.waitForTimeout(1000)
  await tap(page, '.sc-ct__act', '게시판')
  await page.waitForSelector('.sc-er__list', { timeout: 10_000 })
  await page.waitForTimeout(600)
  await page.screenshot({ path: `${OUT}/1-게시판-${tag}.png` })
  console.log(`  찍었다 1-게시판-${tag}.png`)
  await tap(page, '.sc-sheet__panel button', '닫기')
  await page.waitForTimeout(500)

  // ── 점령전. 연구실에서 ──
  await must('openPhase', host, { gameId: game })
  await fund(game, myTeam, 4)
  await giveItems(game, meUid)
  ...
  console.log(`  찍었다 2-점령전-위-${tag}.png`)
  await page.locator('.sc-sheet__body').evaluate((el) => el.scrollBy(0, 1400)).catch(() => undefined)
  await page.waitForTimeout(500)
  await page.screenshot({ path: `${OUT}/3-점령전-아래-${tag}.png` })
  console.log(`  찍었다 3-점령전-아래-${tag}.png`)
  await tap(page, '.sc-sheet__panel button', '닫기')
  await page.waitForTimeout(500)
  ...
  await page.waitForTimeout(600)
  await page.screenshot({ path: `${OUT}/4-제조기-${tag}.png` })
  console.log(`  찍었다 4-제조기-${tag}.png`)
  await ctx.close()
}

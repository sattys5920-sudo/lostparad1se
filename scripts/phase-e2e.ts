...
async function main(): Promise<void> {
   ...
   */
  let clockAt = Number(opened.allInAtMs)
  ...
  check(mid.tileId === 'classroom', `${MOVE_MINUTES}분 뒤에 도착했다`, String(mid.tileId))
  const been = (mid.visitedTiles as string[]) ?? []
  check(been.includes('classroom'), '가 본 방으로 지도에 남는다', `${been.length}곳`)
